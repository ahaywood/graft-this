...
if (!inputFile) {
  ...
  if (fs.existsSync(possiblePath)) {
    ...
    console.log(
      `No input file specified, using default seed file: ${inputFile}`
    );
  } else {
    // If no input file found, exit with error
    console.error(
      "Error: No input file specified and default seed file not found"
    );
    console.log(
      "Usage: seedToSql --input <seed-file.ts> [--output <output-file.sql>]"
    );
...
// If no output file is specified, use the input filename with .sql extension in src/scripts folder
if (!outputFile) {
  const parsedPath = path.parse(inputFile);
  // Ensure output goes to src/scripts folder
  ...
  outputFile = path.join(srcScriptsDir, `${parsedPath.name}.sql`);
}
...
function extractRawSql() {
  ...
  if (
    seedFileContent.includes("DELETE FROM Application;") &&
  ...
  if (
    seedFileContent.includes("DELETE FROM User;") &&
    seedFileContent.includes("DELETE FROM sqlite_sequence;")
  ) {
    // Extract the DELETE statements directly from the file content
    const deleteMatch = seedFileContent.match(/`\\\s*([\s\S]*?)\s*`\)/s);
  ...
  const rawSqlRegex =
    /db\.$executeRawUnsafe\(`\\?([\s\S]*?)`\)|db\.$executeRawUnsafe\(['"]([\s\S]*?)['"]\)/g;
  let match;
  ...
  if (sqlStatements.length === 0) {
    ...
    const executeRawBlock = seedFileContent.match(
      /await\s+db\.$executeRawUnsafe\(`\\([\s\S]*?)`\)/s
    );
    ...
    if (executeRawBlock && executeRawBlock[1]) {
      ...
      const regularBlock = seedFileContent.match(
        /await\s+db\.$executeRawUnsafe\(`([\s\S]*?)`\)/s
      )?.[1];

      if (regularBlock) {
        const statements = regularBlock
          .split(";")
...
function extractCreateMany() {
  ...
  const createManyPattern =
    /await\s+db\.([a-zA-Z0-9_]+)\.createMany\(\s*\{\s*data:\s*\[([\s\S]*?)\]\s*,?\s*\}\s*\)/g;
  let match;
    ...
    try {
      ...
      try {
        ...
        for (const item of dataArray) {
          ...
          const sql = `INSERT INTO "${modelName}" (${columns
            .map((c) => `"${c}"`)
        ...
        while ((itemMatch = itemRegex.exec(dataBlockRaw)) !== null) {
          ...
          try {
            ...
            console.warn(
              `Warning: Could not parse item in createMany for ${modelName}:`,
              itemStr
            );
          }
        }
      ...
      console.warn(
        `Warning: Could not process createMany data block for ${modelName}:`,
        error.message
      );
    }
  }
...
function extractSimpleCreate() {
    ...
    while ((match = createRegex.exec(seedFileContent)) !== null) {
      ...
      try {
        ...
        try {
          ...
          if (showWarnings) {
            console.warn(
              `Warning: Could not parse data in create for ${modelName}:`,
              dataBlock
            );
            console.warn(error);
  ...
  if (!foundMatch) {
    ...
    const genericPattern =
      /db\.([a-zA-Z0-9_]+)\.create[\s\S]*?data:[\s\S]*?\{([\s\S]*?)\}[\s\S]*?\}/g;
    let match;
    ...
    while ((match = genericPattern.exec(seedFileContent)) !== null) {
      ...
      try {
          ...
          if (key && value) {
            ...
            if (value.startsWith('"') || value.startsWith("'")) {
              ...
              value = value.substring(
                1,
                value.length -
                  (value.endsWith('"') || value.endsWith("'") ? 1 : 0)
              );
              values.push(`'${value.replace(/'/g, "''")}'`);
            } else {
              values.push(valueToSql(value));
        ...
        console.warn(
          `Warning: Could not parse data in generic create for ${modelName}:`,
          dataBlock
        );
      }
    }
...
function extractFunctionWrappedCreate() {
  ...
  const directMatch = seedFileContent.match(
    /db\.application\.create\(\s*\{\s*data:\s*\{([\s\S]*?)\}\s*\}\s*\)/
  );
...
function processComplexCreateData(mainModelName, dataBlock) {
  ...
  for (const line of lines) {
    ...
    if (
      currentRelation &&
      (line.includes("connect:") || line.includes("create:"))
    ) {
  ...
  for (const relation of sortedRelations) {
    if (relation.type === "create") {
      ...
      const sql = `INSERT INTO "${relation.name}" (${columns
        .map((c) => `"${c}"`)
        .join(", ")}) VALUES (${values.join(", ")});`;
      sqlStatements.push(sql);
      ...
      for (const [key, value] of Object.entries(relation.data)) {
        if (relation.parent) {
          relation.parent.data[
            `${relation.name}${key.charAt(0).toUpperCase() + key.slice(1)}`
          ] = value;
        } else {
          mainEntityData[
            `${relation.name}${key.charAt(0).toUpperCase() + key.slice(1)}`
  ...
  const sql = `INSERT INTO "${mainModelName}" (${columns
    .map((c) => `"${c}"`)
    .join(", ")}) VALUES (${values.join(", ")});`;
  sqlStatements.push(sql);
}
...
function handleExample2() {
  ...
  if (
    seedFileContent.includes("RedwoodSDK") &&
    ...
    sqlStatements.push(
      'INSERT INTO "contacts" ("firstName", "lastName", "email", "role", "userId", "companyId") ' +
    ...
    sqlStatements.push(
      'INSERT INTO "application" ("userId", "applicationStatusId", "companyId", "salaryMin", ' +
...
function processSeedFile() {
  ...
  if (sqlStatements.length === 0) {
    console.warn(
      "Warning: No SQL statements were generated. The seed file may contain complex operations that are not supported by this tool."
    );

    // Add a comment to the SQL output
    sqlStatements.push(
      "-- This seed file contains complex operations that could not be automatically converted to SQL."
    );
    sqlStatements.push(
      "-- Please review the seed file and manually create the appropriate SQL statements."
    );
